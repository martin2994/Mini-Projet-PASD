...
chunk read_chunk_io ( FILE * f )  {
    ...
    //On met en abstraction tout les espaces et les retours à la ligne
    while(actual_c==(int)' ' || actual_c==(int)'\n'){
      actual_c = fgetc(f);
      if(feof(f)){
        printf("End of File !" );
        break;
        ...
        ***************************************/
        ...
        case( (int) 'p' ):
            ...
            if( 0 == diff_string ){
                ...
                fseek(f, pos_before_c, SEEK_SET);
                fscanf(f, "%s", string_input);
                diff_string = strncmp(string_input, "print_stack", TOKEN_KEYWORD_MAX_LEGTH);
                if( 0 == diff_string ){
                  ...
                  printf("\nOperator_print_stack created !\n");
                    ...
                    fseek(f, pos_before_c, SEEK_SET);
                    fscanf(f, "%s", string_input);
                    diff_string = strncmp(string_input, "print_dictionary", TOKEN_KEYWORD_MAX_LEGTH);

                    if( 0 == diff_string ){
                        //create operator_print_dictionary
                        printf("\nOperator_print_dictionary created !\n");
                    }
                    else{
                        fseek(f, pos_before_c, SEEK_SET);
                        fscanf(f, "%s", string_input);
                        diff_string = strncmp(string_input, "print", TOKEN_KEYWORD_MAX_LEGTH);
        ...
        case( (int) 's' ):
            ...
            if( 0 == diff_string ){
                ...
                fseek(f, pos_before_c, SEEK_SET);
                fscanf(f, "%s", string_input);
                diff_string = strncmp(string_input, "stop_trace", TOKEN_KEYWORD_MAX_LEGTH);
                if( 0 == diff_string ){
        ...
        case( (int) 'i' ):
            ...
            if( 0 == diff_string ){
                ...
                fseek(f, pos_before_c, SEEK_SET);
                fscanf(f, "%s", string_input);
                diff_string = strncmp(string_input, "if_else", TOKEN_KEYWORD_MAX_LEGTH);
                if( 0 == diff_string ){
